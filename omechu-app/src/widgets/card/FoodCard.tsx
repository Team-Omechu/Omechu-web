...
// FSD: widgets에서 entities import 가능
import { Restaurant } from "@/entities/restaurant/model/restaurant.types";
...
export function FoodCard({
  ...
  return (
    <div
      className="border-grey-dark-hover flex items-start justify-between rounded-xl border bg-white p-3 shadow-md"
      onClick={onClick}
    >
      <div className="flex-1">
        ...
        {item.menus && (
          <p className="text-grey-normal-active mb-1 text-sm font-bold">
            {item.menus[0]}
          </p>
        )}
        <div className="mt-1 flex flex-wrap gap-2 text-xs">
          {item.rest_tag?.map((tag: { tag: string }, i: number) => (
            <span
              key={i}
              className="border-grey-normal text-grey-normal rounded-full border px-2 py-0.5"
            >
              {tag.tag}
            </span>
          ))}
      ...
      <div className="flex flex-col place-items-end gap-2">
        ...
        <img
          src={imageSrc}
          alt={item.menus?.[0] ?? "음식"}
          className="border-grey-dark-hover h-18 w-18 rounded-xs border object-cover"
        />
      </div>
    </div>
  );
