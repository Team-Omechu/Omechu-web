        ...
        {Array.from({ length: totalSteps }).map((_, index) => (
          <div
            ...
            className={`h-2.5 flex-1 rounded-3xl border-[1.5px] border-[#00A3FF] ${
              index < currentStep
                ? `hover:bg-secondary-normal-hover active:bg-secondary-normal-active bg-[#00A3FF]`
                : "bg-white"
            }`}
          />
        ))}
      ...
      {onCancelClick && (
        <div className={`flex ${alignClass}`}>
          <button
            ...
            className={`active:bg-secondary-normal-active mt-2 h-6.5 w-auto rounded-md bg-[#00A3FF] px-2.5 pt-2 pb-1.5 text-xs font-light text-white hover:bg-[#0182CA] ${cancelButtonClassName}`}
          >
            {cancelButtonText}
          </button>
        </div>
