    ...
    <div className="mb-2 mt-4 flex justify-end">
      {selectedKeywords.length === 0 ? (
        <button
          ...
          className="flex items-center gap-1 rounded-full border border-grey-dark-hover bg-white px-3 py-1 text-xs text-gray-500"
        >
          추천 키워드
          <Image
        ...
        <div className="flex flex-wrap items-center gap-2">
          {selectedKeywords.map((tag, idx) => (
            <span
              key={tag}
              className="rounded-full border border-grey-dark-hover bg-white px-3 py-1 text-xs text-gray-700"
            >
              {tag}
            </span>
          ))}
